...
# from pymoviezweb.database import db_session
...
def get_needed_fields():
        return ['Title', 'MovieID', 'Medium', 'Year', 'Genre', 'Director',
                'Actor', 'Cover', 'Country', 'Length', 'MPAA', 'Plot',
                'ReleaseDate']
...
def process_xml(xml_data):
    ...
    for movie in root.iter("Movie"):
        ...
        if movieData['MovieID'] in movie_ids:
            logging.warning("Duplicated MovieID found! Please fix: %s"
                            % movieData['Title'])
        else:
...
def process_zip(file_name, output_dir):
    ...
    if xml_file_name not in zfobj.namelist():
        ...
        logging.info("Found %s covers and xml %s" % (cover_count,
                                                     xml_file_path))
...
def get_movie_attribs(movie):
    ...
    textAttributes = ['Title', 'Cover', 'Country', 'Loaned', 'LoanDate',
                      'Length', 'URL', 'MovieID', 'MPAA', 'PersonalRating',
                      'PurchaseDate', 'Seen', 'Rating', 'Status', 'Plot',
                      'ReleaseDate', 'Notes', 'Position', 'Location']
    ...
    if len(unknownTags) > 0:
        logging.info("Unknown or empty tags for movie: %s (%s)"
                     % (movieData['Title'], ', '.join(unknownTags)))

    if movieData['Cover']:
        if not os.path.isfile(os.path.join(scriptPath,
                                           "output/",
                                           movieData['Cover'])):
